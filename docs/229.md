...
例如，如果 f(x) = x <sup>2</sup> 和 *c* = 4，那么*L*c(f)= {-2，2}因为 f(-2) = (-2) <sup>2</sup> = 4 和 f(2) = 2 <sup>2</sup> = 4。
...
要使用 k-NN 密度估计来估计给定点的密度，我们要找到到第 K 个<sup>个</sup>最近点、*d*K 的距离，并以此作为我们邻域的半径。通过这样做，我们得到了一个在我们点周围的邻域，其中正好有其他的点。
...
对于 2 维数据，2 维欧氏球的体积为πR <sup>2</sup> 其中 R = 1，所以*v*d=π，在这种情况下。
