...
现在，假设每个网站都有自己的硬币。网站 A 有一枚硬币以概率 p <sub>A</sub> 正面落地，网站 B 有一枚硬币以概率 p <sub>B</sub> 正面落地。这两种概率我们都不知道，但是我们想确定 p<sub>A</sub>p<sub>B</sub>或者反过来是否成立(也有可能 p <sub>A</sub> = p <sub>B</sub> )。
